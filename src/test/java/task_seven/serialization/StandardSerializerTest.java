...
public class StandardSerializerTest {
    ...
    public void readPublisher() throws Exception {
        Publisher publisher = serializer.readPublisher(filePublisher);
        assertEquals(publishers.get(0), publisher);
    }
    ...
    public static void initData() throws IOException {
        ...
        fileAuthor = new File("D:\\data\\author.dat");
        if (!fileAuthor.exists()) {
            fileAuthor.createNewFile();
        }
        fileBook = new File("D:\\data\\book.dat");
        if (!fileBook.exists()) {
            fileBook.createNewFile();
        }
        filePublisher = new File("D:\\data\\publisher.dat");
